...
def invoke_service(request):
    ...
    response_fromd = requests.get(
        'http://localhost:9004/apid',
        headers=headers,
    )

    headers = {}
    headers.update(create_http_headers_for_new_span())
    response_frome = requests.get(
        'http://localhost:9005/apie',
        headers=headers,
    )
    return Response("service c:" + str(response_fromd.text) + "\t" + str(response_frome.text))

node=ZipkinNode()
node.init_zipkin_settings('Service_C')
node.add_router('invoke_service','/apic')
